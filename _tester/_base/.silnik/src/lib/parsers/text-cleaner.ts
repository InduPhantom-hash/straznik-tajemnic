...
export function sanitizeMechanicalTags(text: string): string {
  ...
  return text
    ...
    .replace(/\[(?:STAN|STAN_POSTACI|HP|MP|SANITY|SAN|POCZYTALNOŚĆ|POCZYTALNOSC|OBRAŻENIA|OBRAZENIA|RANY):[^\]]*\]/gi, '')
    // Walka, obrona, pościgi, magia, tomy
    .replace(/\[(?:WALKA|COMBAT|ATAK_WRĘCZ|ATAK_WRECZ|MELEE_ATTACK|OBRONA|DEFENSE|POŚCIG|POSCIG|CHASE|ZAGROŻENIE|ZAGROZENIE|HAZARD|CZAR|SPELL|MAGIA|TOM|TOME|KSIĘGA|KSIEGA|STUDIUM|OBRONA_MAGIA|MAGIA_OBRONA|OPPOSED_MAGIC|MAGIA_SPONTANICZNA|SPONTANEOUS_MAGIC):[^\]]*\]/gi, '')
