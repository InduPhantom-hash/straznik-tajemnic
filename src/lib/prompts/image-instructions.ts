...
Rutynowe przejścia, oględziny drobiazgów i zwykłe rozmowy zostaw BEZ obrazu - płynność opowieści jest ważniejsza niż liczba ilustracji.${formatBlock}`;
  }
...
Mimo to NIE ilustruj każdej drobnej czynności ani rutynowego przejścia - trzymaj się momentów o realnym znaczeniu.${formatBlock}`;

  if (settings.replicateSettings?.autoGenerateNPCs ?? true)
    instructions += `\n- Priorytetowo ilustruj nowo poznanych, ważnych NPC.`;
