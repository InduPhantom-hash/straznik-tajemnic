...
## Etap 3.5 - Wewnętrzna Encyklopedia, Pomoc & Onboarding

Cel: Zbudowanie wbudowanego Centrum Pomocy i Encyklopedii (wzmocnionej RAG-iem) z zachowaniem pełnego bezpieczeństwa prawnego i izolacji prywatnych materiałów gracza.

### Programistyczne

- [ ] Zbudować komponent modalu pełnoekranowego „Pomoc & Encyklopedia” (Full-screen Modal/Drawer) dostępny z menu i ustawień.
- [ ] Wdrożyć 5 zakładek wspieranych przez lokalną bazę RAG (JSON/SQLite):
  1. 🎮 **Interfejs Gry:** objaśnienie przycisków, wskaźników SAN/HP/MP, rzutów k100 i ekwipunku.
  2. 🎲 **Jak grać?:** onboarding i poradnik pętli gry z AI dla osób początkujących.
  3. 📚 **Bestiariusz & Lore:** lokalna encyklopedia mitów z wyszukiwarką na bazie `Cthulhu-Wiki-KB`.
  4. 📜 **Zasady & Mechanika:** skrót reguł CoC 7e, testów forsowanych, poczytalności i walki.
  5. ℹ️ **Informacje & Prawa Autorskie:** nota prawna Public Domain Lovecrafta + Fan Policy Chaosium.
- [ ] Izolacja i obsługa dozwolonego użytku: odróżnienie wbudowanej bazy wiedzy (Public Domain) od prywatnego RAG dla wgranych plików PDF gracza (z prawem do dosłownego cytowania stron w prywatnej sesji).
- [ ] Dodać interaktywnego asystenta pomocy w modalu (pytania w języku naturalnym o zasady i interfejs).

### Nieprogramistyczne

- [ ] Wyczyszczenie i adaptacja wbudowanego zbioru danych z `Cthulhu-Wiki-KB` (fandom wiki, relacje grafu wiedzy) do formatu bazy aplikacji.
- [ ] Przygotowanie przystępnych opisów UI oraz poradnika „Jak grać” w cosmic horror RPG.
- [ ] Rozbudowa Bazy Epok (od końca XIX w. do współczesności): przetwarzanie surowego researchu w syntetyczne opracowania tła społecznego, kultury, prawa, obyczajów, technologii i wierzeń (bez kopiowania tekstów 1:1, z zachowaniem praw autorskich).
- [ ] Opracowanie profilu Postaci Historycznych: wierne odwzorowanie faktów biograficznych + opcjonalne warstwy nadprzyrodzone/fabularne na potrzeby scenariuszy.
- [ ] Zweryfikowanie zgodności z licencją Public Domain oraz Chaosium Fan Material Policy.
...
## Pomysły i rozszerzenia w tle (Future Backlog)

### Rozwój trybu Hot Seat (2-4 graczy)

- [ ] Umożliwienie prowadzenia gry w trybie Hot Seat dla grupy 2-4 graczy (przekazywanie urządzenia/tury).
- [ ] Obsługa osobnych kart postaci, indywidualnych statystyk i dyskretnych widoków dla każdego z graczy.
- [ ] Przystosowanie narracji AI (MG) do kierowania scenami z udziałem wielu badaczy jednocześnie.

*Uwaga: Zaczepka informacyjna na przyszłość, wymaga najpierw stabilizacji Etapów 1-3.*
