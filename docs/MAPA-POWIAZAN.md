...
| **`.agent/plans/dziennik-ekwipunek-postacie-plan.md`**<br/>*(Plan Przebudowy Dziennika, Ekwipunku i Biografii)* | • 2-kolumnowy layout detalu przedmiotu i placeholdery SVG per kategoria<br/>• Zunifikowany widok Odkryć z 4 kategoriami (Miejsca, Postacie, Przedmioty, Misje)<br/>• Ciemne scrollbary Art-Deco `.journal-scroll` i 4 główne zakładki Dziennika<br/>• Rozbudowane biografie 30 postaci A4 z kluczowymi więziami fabularnymi | • `src/components/ui/equipment-detail-dialog.tsx`<br/>• `src/components/ui/equipment-image-placeholder.tsx`<br/>• `src/components/ui/journal/discoveries-view.tsx`<br/>• `src/components/ui/session-journal.tsx`<br/>• `src/components/ui/equipment-modal.tsx`<br/>• `src/lib/immersion/predefined-characters.ts`<br/>• `src/app/globals.css` |
