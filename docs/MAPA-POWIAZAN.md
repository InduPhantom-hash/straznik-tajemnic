...
| **`docs/ROADMAP-MECHANIKI-AI.md`**<br/>*(Roadmapa rozwoju)* | • Lokalny RAG jako źródło prawdy<br/>• Domknięcie "Koniec Sesji"<br/>• Lokalny pipeline przygody<br/>• Immersja i assety<br/>• Adventure Creator i Quick Start<br/>• i18n PL/EN | • `src/lib/pacing-controller.ts`<br/>• `src/components/sidebar/CthulhuSidebar.tsx`<br/>• `src/components/chat/chat-window/components/message-input.tsx`<br/>• `src/lib/vector-db/local-vector-store.ts`<br/>• `src/lib/adventures-data.ts` |
