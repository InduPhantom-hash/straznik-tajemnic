...
graph TD
    ...
    subgraph Komponenty_UI["Komponenty UI & Hooki (src/components & src/hooks)"]
        ...
        HELP_MODAL["src/components/ui/app-help-modal.tsx<br/>src/lib/data/app-help-data.ts"]
    end
    ...
    UGUIDE -->|Encyklopedia Aplikacji & Przewodnik Gracza| HELP_MODAL
    ROADMAP -->|Przycisk Koniec Sesji & Pomoc| SIDEBAR
...
| :--- | :--- | :--- |
| **`state.md`**<br/>*(Single Source of Truth / Tracker Projektu)* | • Tablica statusów zadań Linear-style<br/>• Stan Etapów 0-6, Etapu 0.5 (Onboarding) i Etapu 3.5 (Encyklopedia Aplikacji)<br/>• Mapa powiązań modułów i RAG assets | • `docs/ROADMAP-MECHANIKI-AI.md`<br/>• `docs/ARCHITECTURE.md`<br/>• Wszystkie pliki `src/` wymienione w trackerze |
...
| **`.agent/plans/app-help-plan.md`**<br/>*(Plan Encyklopedii Aplikacji & Przewodnika Gracza)* | • Modal z 3 zakładkami: Elementy UI, Wirtualny Strażnik Tajemnic AI (CoC 7E) oraz Poradnik Gracza & Protipy<br/>• Instrukcja rozmawiania z czatem AI, budowanie klimatu, opisy sensoryczne, zasada "graj jak lubisz"<br/>• Przycisk Pomoc w Sidebarze z ikoną `HelpCircle` | • `src/lib/data/app-help-data.ts`<br/>• `src/components/ui/app-help-modal.tsx`<br/>• `src/components/sidebar/CthulhuSidebar.tsx` |
...
| **`docs/USER_GUIDE.md`**<br/>*(Przewodnik Gracza)* | • Kreator badacza (charakterystyki, zawód)<br/>• Tacka na Kości (k100, progi, Push Roll, Szczęście)<br/>• Tryb Hot Seat (1-2 graczy, spójny start i pytania otwarcia)<br/>• Zapis i wczytanie sesji<br/>• Trwała Tablica Badacza (płótno korkowe 2400x1600, sznurki Beziera SVG z grawitacją, Szuflada Poszlak, Lightbox)<br/>• Encyklopedia Aplikacji & Przewodnik Gracza (UI, AI MG, Protipy RP) | • `src/components/character-sheet/`<br/>• `src/components/dice/dice-tray.tsx`<br/>• `src/lib/dice-utils.ts`<br/>• `src/components/chat/hot-seat-banner.tsx`<br/>• `src/hooks/useGameStart.ts`<br/>• `src/components/ui/journal/corkboard-investigation-board.tsx`<br/>• `src/components/dialogs/inspection-lightbox-modal.tsx`<br/>• `src/types/investigator-board.ts`<br/>• `src/components/ui/session-journal.tsx`<br/>• `src/components/ui/app-help-modal.tsx`<br/>• `src/lib/data/app-help-data.ts` |
| **`docs/ROADMAP-MECHANIKI-AI.md`**<br/>*(Roadmapa rozwoju)* | - Lokalny RAG jako zrodlo prawdy<br/>- Domkniecie "Koniec Sesji"<br/>- Lokalny pipeline przygody<br/>- Immersja i assety<br/>- Adventure Creator i Quick Start (Karty Autorskich Scenariuszy Strefy 11 w `adventure-selector.tsx`) <br/>- Onboarding & Quick Setup Flow (Etap 0.5)<br/>- i18n PL/EN | - `src/lib/pacing-controller.ts`<br/>- `src/components/sidebar/CthulhuSidebar.tsx`<br/>- `src/components/chat/chat-window/components/message-input.tsx`<br/>- `src/lib/vector-db/local-vector-store.ts`<br/>- `src/lib/adventures-data.ts`<br/>- `_tester/_base/.silnik/src/components/ui/adventure-selector.tsx`<br/>- `src/app/api/chat/_helpers/build-immersion-context.ts`<br/>- `src/lib/immersion/astronomy-service.ts`<br/>- `src/lib/immersion/news-service.ts`<br/>- `src/lib/immersion/pricing-service.ts`<br/>- `src/components/onboarding/` |
...
| **`.agent/plans/dziennik-ekwipunek-postacie-plan.md`**<br/>*(Plan Przebudowy Dziennika, Ekwipunku i Biografii)* | • 2-kolumnowy layout detalu przedmiotu i placeholdery SVG per kategoria<br/>• Zunifikowany widok Odkryć z 4 kategoriami (Miejsca, Postacie, Przedmioty, Misje)<br/>• Ciemne scrollbary Art-Deco `.journal-scroll` i 4 główne zakładki Dziennika<br/>• Rozbudowane biografie 40 postaci z kluczowymi więziami fabularnymi<br/>• Zunifikowany mroczny Design System Satiszaid (`docs/assets/design-system/`) w 100% zaimplementowany w Dzienniku Sesji i Tablicy Badacza | • `src/components/ui/equipment-detail-dialog.tsx`<br/>• `src/components/ui/predefined-characters-selector.tsx`<br/>• `src/components/ui/investigator-board.tsx`<br/>• `src/components/ui/session-journal.tsx`<br/>• `src/lib/immersion/predefined-characters.ts`<br/>• `src/app/globals.css`<br/>• `docs/assets/design-system/` |
| **`docs/entity-mapping.ts (SSOT)`**<br/>*(System Typów i Tagów Dziennika)* | • Single Source of Truth dla mapowania surowych tagów z modelu AI (np. `npc`, `location`) na odpowiednie typy Dziennika (`encyclopedia_character`) i Tablicy (`suspect`)<br/>• Odporność na literówki i zmiany struktury RAG | • `_tester/_base/.silnik/src/lib/journal/entity-mapping.ts`<br/>• `_tester/_base/.silnik/src/lib/journal/convert-entries.ts`<br/>• `_tester/_base/.silnik/src/lib/journal-storage.ts` |
